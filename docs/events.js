let scrollY = 0; // Store scrollY globally to restore properly
...
function disableScroll() {
    ...
    document.body.style.left = '0'; // optional but helpful
...
function enableScroll() {
    ...
    window.scrollTo(0, scrollY); // restore scroll position correctly
}
...
window.closePopup = function(id) {
    ...
    if (popup) {
        ...
        dimmer.classList.add('hidden'); // hide the dimmer
    }
}
...
window.openPopup = function(id) {
    ...
    if (popup) {
        ...
        }, 10); // allow time for the transition to kick in
...
function renderPopup(event) {
  ...
  popup.className = `popup fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90vw] md:w-[50vw] h-[90vh] md:h-[70vh] bg-white dark:bg-gray-800 backdrop-blur-3xl bg-opacity-75 rounded-xl shadow-xl z-50 scale-0 transition-transform duration-300 ease-in-out overflow-hidden flex flex-col`;
  ...
  popup.innerHTML = `
    ...
    <div class="px-10 py-4 overflow-y-auto flex-1">
